...
def tensor_to_image(tensor, cmap=None):
    ...
    # Overlay the matrix elements on the image
    tensor_np = tensor.squeeze().numpy()
    for (x, y), val in np.ndenumerate(tensor_np):
        plt.text(y, x, f'{val:.2f}', ha='center', va='center', color='black', fontsize=8)
...
# Save channel images in original visualization style with matrix elements
