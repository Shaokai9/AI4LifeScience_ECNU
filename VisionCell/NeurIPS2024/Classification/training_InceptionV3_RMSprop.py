...
if __name__ == "__main__":
    ...
    criterion = initialize_loss_function(args.loss_function)
    optimizer = optim.RMSprop(model.parameters(), lr=args.learning_rate, momentum=args.momentum, alpha=0.9, eps=1e-08)
    ...
    log_filename = f"InceptionV3_Base_log_{run_timestamp}.txt"
    ...
    best_model_name = f"trained_InceptionV3_Base_bestmodel_{run_timestamp}.pth"
    ...
    plot_loss(epochs, train_loss_list, val_loss_list, 'Loss', 'Epoch', f'InceptionV3_Base_training_validation_loss_{run_timestamp}.png') #

    plot_all_metrics(epochs, accuracy_list, f1_list, specificity_list, precision_list, recall_list, "Performance Over Epochs", f'InceptionV3_Base_all_metrics_{run_timestamp}.png')
    ...
    for metric, metric_name in zip(metrics_for_comparison, metric_names_for_comparison):
        plot_metrics_vs_f1(metric, f1_list, metric_name,
                           f'{metric_name} vs F1-Score', f'InceptionV3_Base_{metric_name}_Vs_F1-Score_{run_timestamp}.png')
