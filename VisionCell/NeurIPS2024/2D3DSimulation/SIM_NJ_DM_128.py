...
# We can adjust the num_workers value based on your system's capabilities and the number of available CPU cores.
#  Using a high number of worker threads might increase the CPU and memory usage. It is essential to strike a balance between improving data loading performance and the available system resources.
...
# Resize the generated images to the desired dimensions (1155x1609)
resized_images = [Image.fromarray((img * 255).astype('uint8')).resize((1609, 1155), Image.ANTIALIAS) for img in sampled_images]
